...
defmodule AshR2ml.Ingestion do
  ...
  consumed by `AshR2ml.Admission`. It deliberately does not reason over arbitrary
  OWL. Missing closure information is returned as a typed refusal.

  Ash-specific relational hints live in the neutral AshR2ML vocabulary:
  `https://seanchatmangpt.github.io/ash_r2ml#`.
  """
  ...
  @r2ml "https://seanchatmangpt.github.io/ash_r2ml#"
  ...
  @spec compile_turtle(String.t(), keyword()) ::
          {:ok, AshR2ml.Compilation.t()} | {:error, AshR2ml.Compilation.t() | [Refusal.t()]}
  def compile_turtle(turtle, opts \\ []) do
    with {:ok, profile} <- from_turtle(turtle, opts) do
      AshR2ml.Compiler.compile(profile)
    end
  end
