...
defmodule AshR2ML.SPARQL.DifferentialReceipt do
  ...
  @type t :: %__MODULE__{
          ...
          strategies: [AshR2ML.SPARQL.Plan.strategy()],
...
defmodule AshR2ML.SPARQL.Differential do
  @moduledoc """
  ...
  alias AshR2ML.SPARQL.{DifferentialReceipt, Observation, Result}
  ...
  @spec require_strategies(DifferentialReceipt.t(), [AshR2ML.SPARQL.Plan.strategy()]) ::
          :ok | {:error, Refusal.t()}
    ...
    if Enum.all?(observations, &match?(%Observation{}, &1)) do
      ...
      {:error,
       Refusal.new(
         :REFUSED_UNPROVEN_EQUIVALENCE,
         :sparql_differential,
         "all differential inputs must be AshR2ML.SPARQL.Observation values"
       )}
    end
  end
